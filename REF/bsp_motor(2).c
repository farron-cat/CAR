...
#define MOTOR_MODE  1   // 驱动方式选择     0:互补输出   1: 单路输出
...
void Motor_Init(void)
{
    ...
    PWMx_InitDefine PWMx_InitStruct = {0};
    ...
    M1_P1 = 0; M1_P2 = 0;   M2_P1 = 0; M2_P2 = 0;
    M3_P1 = 0; M3_P2 = 0;   M4_P1 = 0; M4_P2 = 0;

#if MOTOR_MODE == 0     // 互补通道模式
    PWMx_InitStruct.PWM_Mode = CCMRn_PWM_MODE1;  // 使用模式1，duty越大，占空比越大
#elif MOTOR_MODE == 1   // 单通道模式
    PWMx_InitStruct.PWM_Mode = CCMRn_PWM_MODE2;  // 使用模式2，duty越大，占空比越小
#endif

    PWMx_InitStruct.PWM_Duty = PWM_DUTY_MAX>>1;      // 初始化为互补输出，取中值防止提前启动电机
    PWMx_InitStruct.PWM_EnoSelect = ENO1P|ENO1N;
    PWM_Configuration(PWM1, &PWMx_InitStruct);
    PWMx_InitStruct.PWM_EnoSelect = ENO2P|ENO2N;
    PWM_Configuration(PWM2, &PWMx_InitStruct);
    PWMx_InitStruct.PWM_EnoSelect = ENO3P|ENO3N;
    PWM_Configuration(PWM3, &PWMx_InitStruct);
    PWMx_InitStruct.PWM_EnoSelect = ENO4P|ENO4N;
    PWM_Configuration(PWM4, &PWMx_InitStruct);
    // 配置PWM总参数
    PWMx_InitStruct.PWM_DeadTime = 5;                //死区发生器设置, 0~255
    PWMx_InitStruct.PWM_Period = PWM_DUTY_MAX - 1;   //周期时间，10kHz
    PWMx_InitStruct.PWM_MainOutEnable = ENABLE;      //主输出使能
    PWMx_InitStruct.PWM_CEN_Enable = ENABLE;         //使能计数器
    PWM_Configuration(PWMA, &PWMx_InitStruct);
...
void Motor_SetAll(int LeftFront, int RightFront, int LeftRear, int RightRear)
{
    ...
    // 左前
    LeftFront = LeftFront>(-PWM_DUTY_MID)?LeftFront:(-PWM_DUTY_MID);    // 限幅,大于-600
    LeftFront = LeftFront<PWM_DUTY_MID?LeftFront:PWM_DUTY_MID;          // 限幅,小于600
    tmp = (uint16)(LeftFront + 600);
    if (tmp == PWM_DUTY_MID) {
        PWMA_CC4E_Disable();            // 关闭通道输出
        PWMA_CC4NE_Disable();
    ...
    // 右前
    RightFront = RightFront>(-PWM_DUTY_MID)?RightFront:(-PWM_DUTY_MID);    // 限幅,大于-600
    RightFront = RightFront<PWM_DUTY_MID?RightFront:PWM_DUTY_MID;          // 限幅,小于600
    ...
    // 左后
    LeftRear = LeftRear>(-PWM_DUTY_MID)?LeftRear:(-PWM_DUTY_MID);    // 限幅,大于-600
    LeftRear = LeftRear<PWM_DUTY_MID?LeftRear:PWM_DUTY_MID;          // 限幅,小于600
    ...
    // 右后
    RightRear = RightRear>(-PWM_DUTY_MID)?RightRear:(-PWM_DUTY_MID);    // 限幅,大于-600
    RightRear = RightRear<PWM_DUTY_MID?RightRear:PWM_DUTY_MID;          // 限幅,小于600
